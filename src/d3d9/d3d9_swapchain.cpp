...
static bool War3UseFpsUnlockOnlyMode() {
  static const bool s_fpsUnlockOnly =
      dxvk::war3::runtime::UseDxvkOnlyCompatibilityMode();
  return s_fpsUnlockOnly;
}

static LRESULT CallWar3Proc(HWND hWnd, WNDPROC proc, UINT msg, WPARAM wParam, LPARAM lParam) {
  const bool unicode = IsWindowUnicode(hWnd) != FALSE;
  return dxvk::CallCharsetFunction(
      CallWindowProcW, CallWindowProcA, unicode, proc, hWnd, msg, wParam, lParam);
}

static LRESULT CallWar3DefProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  const bool unicode = IsWindowUnicode(hWnd) != FALSE;
  return dxvk::CallCharsetFunction(
      DefWindowProcW, DefWindowProcA, unicode, hWnd, msg, wParam, lParam);
}

static LRESULT ForwardWar3Proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  if (g_War3WndProc)
    return CallWar3Proc(hWnd, g_War3WndProc, msg, wParam, lParam);
  return CallWar3DefProc(hWnd, msg, wParam, lParam);
}

[[maybe_unused]] static void InstallWar3WndProcHook(HWND hWnd) {
  if (!hWnd || g_War3WndProc)
    return;
  const bool unicode = IsWindowUnicode(hWnd) != FALSE;
  g_War3WndProc = reinterpret_cast<WNDPROC>(dxvk::CallCharsetFunction(
      SetWindowLongPtrW, SetWindowLongPtrA, unicode, hWnd, GWLP_WNDPROC,
      reinterpret_cast<LONG_PTR>(War3WndProcHook)));
  dxvk::Logger::info(dxvk::str::format("War3WndProcHook installed on HWND: ",
                                       hWnd));
}

static void UninstallWar3WndProcHook(HWND hWnd) {
  if (!hWnd || !g_War3WndProc)
    return;
  const bool unicode = IsWindowUnicode(hWnd) != FALSE;
  dxvk::CallCharsetFunction(SetWindowLongPtrW, SetWindowLongPtrA, unicode, hWnd,
                            GWLP_WNDPROC,
                            reinterpret_cast<LONG_PTR>(g_War3WndProc));
  g_War3WndProc = nullptr;
}
...
static bool ShouldKeepPerfRecordingOnGameStart() {
  const char* keep = std::getenv("DXVK_WAR3_PERF_KEEP_RECORDING_ON_GAME_START");
  if (keep && keep[0] != '\0' && keep[0] != '0')
    return true;

  // 自动化常用：若通过环境变量开启“启动即录制”，默认不在进图后重置。
  const char* autoRecord = std::getenv("DXVK_WAR3_PERF_RECORD_ON_START");
  if (autoRecord && autoRecord[0] != '\0' && autoRecord[0] != '0')
    return true;

  return false;
}

static bool ShouldStartPerfRecordingOnGameStart() {
  const char* start = std::getenv("DXVK_WAR3_PERF_RECORD_AFTER_GAME_START");
  return start && start[0] != '\0' && start[0] != '0';
}

static LRESULT CALLBACK War3WndProcHook(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (War3UseFpsUnlockOnlyMode())
        return ForwardWar3Proc(hWnd, msg, wParam, lParam);
    if (dxvk::war3::runtime::IsWar3RuntimeModuleDisabled(
    ...
    if (msg == WM_CLOSE || msg == WM_DESTROY || msg == WM_NCDESTROY) {
        // 窗口销毁/重建过程由游戏与 DXVK 自己收口。
        // 这里额外投递 WM_QUIT 会把“分辨率切换中的临时销毁”误当成整个进程退出，
        // 导致切分辨率后直接卡死/退进程。
        return ForwardWar3Proc(hWnd, msg, wParam, lParam);
    }

    // Forward to ImGui Manager
    using namespace dxvk::war3;
    if (msg == WM_KEYDOWN && wParam == VK_F1 && (GetKeyState(VK_CONTROL) & 0x8000)) {
    ...
    // IME 消息链必须优先交给游戏窗口过程，避免输入法结果串被 UI 层提前吞掉。
    if (IsImeMessage(msg) || (msg == WM_KEYDOWN && wParam == VK_PROCESSKEY))
        return ForwardWar3Proc(hWnd, msg, wParam, lParam);

    bool handled = War3Imgui::get().handleWndProc(hWnd, msg, wParam, lParam);
    ...
    const bool isMouseMsg =
        ...
        msg == WM_MBUTTONDBLCLK || msg == WM_MOUSEWHEEL || msg == WM_MOUSEHWHEEL ||
...
namespace dxvk {
  ...
  D3D9SwapChainEx::D3D9SwapChainEx(
    ...
    // Windowed / fullscreen 统一走 D3D9WindowProc，
    // 避免 maximize / live reset 落入不同 wndproc 特例路径。
  ...
  D3D9SwapChainEx::~D3D9SwapChainEx() {
    ...
    // Uninstall War3 WndProc Hook
    UninstallWar3WndProcHook(m_window);
  ...
  HRESULT STDMETHODCALLTYPE D3D9SwapChainEx::Present(
    ...
    if (!War3UseFpsUnlockOnlyMode() &&
        ...
        war3::War3Events::get().registerOnGameStart([]() {
            if (ShouldStartPerfRecordingOnGameStart()) {
              war3::War3PerfMonitor::instance().resetHistory();
              war3::War3PerfMonitor::instance().setRecording(true);
            } else if (!ShouldKeepPerfRecordingOnGameStart()) {
              war3::War3PerfMonitor::instance().setRecording(false);
              war3::War3PerfMonitor::instance().resetHistory();
            }
            // MPQ 读取测试 - 游戏开始后执行
            Logger::info("[FileManager Test] OnGameStart triggered, testing MPQ read...");
            
            std::vector<uint8_t> buffer;
            const std::string testPath = "war3mapImported\\Disaster.blp";
            
            if (war3::FileManager::get().readFromMpq(testPath, buffer)) {
                Logger::info(str::format(
                    "[FileManager Test] SUCCESS: Loaded '", testPath, "' from MPQ, size=", buffer.size(), " bytes"
                ));
                
                // 检查 BLP 文件头魔数 (BLP1 或 BLP2)
                if (buffer.size() >= 4) {
                    char magic[5] = {0};
                    std::memcpy(magic, buffer.data(), 4);
                    Logger::info(str::format("[FileManager Test] File magic: '", magic, "'"));
                }
            } else {
                Logger::warn(str::format(
                    "[FileManager Test] FAILED: Could not load '", testPath, "' from MPQ"
                ));
            }
        });

        war3::War3Events::get().registerOnGameExit([]() {
            war3::War3PerfMonitor::instance().setRecording(false);
        });
    }
    ...
    try {
      ...
      if (!War3UseFpsUnlockOnlyMode() &&
        ...
        // [War3] Fallback render if not handled before UI
        if (!war3::War3Imgui::get().hasRenderedThisFrame()) {
  ...
  HRESULT D3D9SwapChainEx::Reset(
    ...
    if (m_window != m_presentParams.hDeviceWindow) {
      m_window = m_presentParams.hDeviceWindow;
      m_displayRefreshRateDirty = true;
    }
  ...
  void D3D9SwapChainEx::PresentImage(UINT SyncInterval) {
    ...
    if (status >= 0 && status != VK_NOT_READY) {
      ...
      if (m_presentParams.Windowed) {
        const VkExtent2D trackedViewportExtent =
          m_parent->GetFrameMaxViewportExtent();
        const bool viewportCaughtUp =
          trackedViewportExtent.width >= srcRect.extent.width &&
          trackedViewportExtent.height >= srcRect.extent.height;

        if (viewportCaughtUp && HasWindowedLogicalSourceExtent()) {
          SetWindowedLogicalSourceExtent(srcRect.extent, false);
        }

        const VkExtent2D logicalExtent =
          HasWindowedLogicalSourceExtent()
            ? GetWindowedLogicalSourceExtent()
            : trackedViewportExtent;
        if (logicalExtent.width > 0u && logicalExtent.height > 0u &&
            logicalExtent.width <= srcRect.extent.width &&
            logicalExtent.height <= srcRect.extent.height &&
            (logicalExtent.width != srcRect.extent.width ||
             logicalExtent.height != srcRect.extent.height)) {
          srcRect = {
              {0, 0},
  ...
  HRESULT D3D9SwapChainEx::EnterFullscreenMode(
    ...
    // 切入 fullscreen 前先卸掉 windowed 专用 War3WndProcHook，
    // 保持 live reset / 改分辨率时只有一层 D3D9WindowProc。
    UninstallWar3WndProcHook(m_window);
  ...
  HRESULT D3D9SwapChainEx::LeaveFullscreenMode() {
    ...
    // 返回 windowed 后恢复统一 D3D9 wndproc 链。
    HookWindowProc(m_window, this);
