...
内存维护修复版。用户对相同源码候选完成一轮试玩并反馈未见问题；正式构建另更新产品/JAPI版本
与匹配的版本探针，不添加新优化。Shader API 1.2.0、JASS wire v1不变。

### 动态预算与安全回收

- 快照/Arena统一检查实际显存堆预算、含分配器缓存的物理后备占用及32位进程地址空间余量。
  保留安全余量；预算下降时阻止继续扩页，不抢回在途资源。驱动预算不可用时保守回退。
- 快照池在可信预算下默认最多512 MiB，显式配置上限仍有效，不支持预算时回退384 MiB。
  Arena仍为每代384/总1152 MiB。额度不是预分配，更不是整张显卡的显存上限。
- 同类页中的少量存活切片不再必然阻止整页剩余区间复用：缓存、排队复制和实际消费者共同保留
  切片租约，最后使用者释放后才能复用空洞。覆盖阴影、轮廓、GPU输入取证和同步Shader回调。
  不移动存活字节、不削减必需caster；元数据耗尽或不连续碎片仍有保守回退。
- Arena在已完成的代际切换点，依据需求窗口逐步释放空闲尾页。DXVK/驱动物理缓存可能稍后才下降。
- 内存压力下同帧重复失败扩页采用负结果节流，避免每个被拒请求都查询驱动；较小请求、下一帧
  以及已有合法尾部/空洞不因此被永久阻断。Render Stats补充最近扩页额度、复用量与拒绝次数。

### 分配失败与截图维护

- 补齐VkImage创建成功后、allocation接管前的异常清理；普通/导入图像在初始化完成后再登记，
  避免构造失败残留句柄或登记。设备丢失不被吞成普通预算拒绝。
- D3D9内存块保留小于4 KiB的非零尾段；零长度及对齐溢出提前拒绝，异常路径保留正确所有权。
- 无截图请求时不预分配三槽映射读回缓冲；完成后的空闲槽由Present所有者回收自身引用，
  不提前回收Submitted/Quarantined数据。2560×1440下可避免42.1875 MiB逻辑读回槽预热，
  这不是整进程或整张显卡的实测节省量。

### 验证与保留边界

- 对应候选：断言开启CPU测试99/99、静态脚本275/275、切片/预算生产组件23,965检查；
  玩家单轮试玩反馈良好。正式版本标识构建和包身份另行核验，不把旧DLL摘要挪作新DLL验收。
- 未承诺新增FPS比例、所有GPU永不OOM、长时压力及同进程跨地图全部稳定。Issue #8继续回归。
- 诊断跨attempt的Recovered归因与同步慢磁盘关闭仍待后续修复；不把它们描述为本版已解决。
- 不包含64位渲染产品、未完成Water、自动原版模型灯或未批准剔除实验。

## v1.22.00 - 2026-09-20

本版集中改善帧吞吐、大地图阴影资源恢复、蒙皮来源交接和诊断可用性。产品/JAPI显示版本为
1.22.00；Shader API仍为1.2.0，JASS协议仍为warvk:v1。Froxel High、局部雾与方向Guide
继承自1.21，不重复列为新增功能。用户完成RC试玩并授权发布；最终另含有界的统计显示修正。

### 普通帧同步与异步截图

- 对已验证、无像素消费者的原生帧尾同步路径，跳过无用后缓冲读回，解除等待GPU的阻塞。
  这不是删除真实资源fence或Present同步，也不依赖性能日志是否正在录制。
- 原实现每帧请求LockRect，即使随后不读取像素，DXVK仍需准备可读数据并等待GPU完成。
  曾观测到的约4.7ms是资源就绪等待，不是暴雪按FPS算出的固定sleep，也不是单纯复制成本。
  跳过无消费者读回让CPU准备与GPU绘制重新重叠，提高流水线吞吐；不等于着色器少算或输入延迟必然下降。
- 历史同DLL、同图、隔离2560×1440 ABBA：Present均值13.154779→7.840445ms，间隔减少
  40.40%，倒数吞吐76.018→127.544帧/秒（+67.78%）。这些是历史策略对照，不是正式
  v1.21→v1.22普遍FPS同比；玩家另报告高压图约40%提升，不外推其他配置。
- 真正截图改为有界异步队列、三槽GPU复制、精确完成信号后后台像素处理与文件保存，保留TGA。
  不通过“两三帧后肯定完成”的猜测映射数据；满队列/失败、Reset和设备丢失保持显式处理。
- 不再因普通诊断输出自动弹出控制台；修正管道线程与性能TLS退出期处理，避免已知卸载路径异常。

### 大地图快照内存与阴影恢复

- position/UV/index快照按长期、短期与未知保留意图优先分组装页，减少少量长期缓存钉住
  整页已过期临时切片。受限时仍可使用合法页尾，不增加默认384MiB，也不删必需caster凑通过。
- 失败捕获不再刷新成功缓存寿命，解除不再使用的backing/UV引用并改进内存账户。
- 加固buffer初始allocation失败、完全初始化前注册与构造异常清理；设备丢失不被吞成容量回退。
- 玩家此前复现的“走过其他区域后特定地点持续无影”在修复候选复测中未再观察到；预算仍有上限，
  不承诺任意大工作集都能容纳，不以此结论销账所有历史闪退或同进程跨地图问题。

### 短时阴影撕裂／细长暗条（Issue #8）

- 这不是显示器扫描撕屏。坏帧顶点、索引、蒙皮组与矩阵输入已复算出异常拉长三角形；
  同一错误世界角点可同时进入surface和volume-sun，解释了体积雾里也出现异常暗影的现象。
- 修复当前槽位无效时重读记忆旧槽位并冒充新数据的路径；严格路径采用当前身份、帧、槽位及
  发布代际匹配的复制快照，冷/热缓存都检查矩阵组数覆盖。
- 矩阵与实际来源、坐标空间及有效期一起交接；替换后复核，避免世界空间矩阵重复叠乘对象变换。
  无法证明时拒绝本次使用，不钳制骨骼编号、不截短异常边或关闭全部动态投影来隐藏问题。
- **异常输入及相关代码缺陷已定位，修复纳入本版，间歇性视觉问题继续回归。** 阶段性玩家未见
  撕裂不等于所有地图完全根治；原生槽位具体重分配所有者、逐像素归因等仍未全部证明。

### JASS与体积效果

- 修正太阳关闭语义：停止直射及太阳阴影，环境/自发光保留，而非仅撤销WarVK覆盖后恢复原生太阳。
- 修正1.27a LoadInteger/LoadReal typed carrier签名，保持精确检查与字符串协议回退。
- 延续局部雾三形、8槽上限、Froxel Medium/High及Guide，并补限定场景参数启停/修改/销毁验证。
  历史83断言覆盖49个公开函数的限定用例，不冒充全部109个命令的运行效果验收。
- 作者包保留能力查询与明确UnsupportedFeature语义；原版模型灯接管研究接口不作为本版支持承诺。

### Render Stats与工程收尾

- Ctrl+F1 Render Stats由占位改为4Hz按需读取快照页、Arena、预算、进程VA及生产/回放/接收状态。
- 修正帧统计清零但未分配的缓存命中帧显示“快照0MiB”；所有scene轮转重取存活池统计，回收时
  同步更新页内占用。真实空池仍为0，不用历史最大值掩盖真实释放。
- Arena显示上一提交代使用量、当前代瞬时量与保留容量；明确其高水位页不会自动缩容。
  仅改统计，不修改allocation、fence或caster策略；不同来源仍不构成一致事务快照。
- 加入同根DataCollection调用树及主线程帧时间线，区分执行/等待/未覆盖区间；不是全线程采样栈分析器。
- 加固所有者线程推进、工作指针与代际绑定、按值进度发布；共享palette内核、复用输出容量及公共shader逻辑。
- 正式默认关闭重型帧取证，保留蒙皮正确性合同；版本/探针一致，MinHook从固定源码构建，冻结测试
  参考字节不再被Git自动换行破坏。长期M3/M4、跨Pass资源所有权及所有剔除工作未宣称完成。

### 已知限制与排除项

- 一次新进程只进入一张地图；同进程换图不承诺稳定。
- 有限快照/Arena预算、Arena暂不自动缩容；极端Froxel请求可回退Legacy，细小轮廓仍需视觉评估。
- 不包含未完成Water改造、原版模型自动点光接管、64位渲染产品及未验收Consume实验。
- 本轮玩家接受属于实际试玩反馈，不伪造新组合全量GPU/长时/API/性能同比数据；旧失败证据保留。
