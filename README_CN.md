...
- 高单位密度区域压低镜头时，太阳阴影候选、蒙皮与四级联工作量可能超过安全预算。不完整的 CSM
  会被拒绝发布，表现为阴影闪烁或暂时不显示，但准备成本仍然存在；当前不会通过降低 4096 CSM、
  扩大 Arena 或放宽资源验证来掩盖问题。进度见 [#5](https://github.com/CallDisaster/War3VK/issues/5)。
...
- 上述问题计划在后续版本继续修复；同进程跨地图和点阴影伪影分别跟踪于 [#6](https://github.com/CallDisaster/War3VK/issues/6) 与 [#7](https://github.com/CallDisaster/War3VK/issues/7)。“一次启动只游玩一张地图”仍是 1.2.0 系列推荐的使用方式。
