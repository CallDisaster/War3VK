...
集成说明：Guide-aware composite 的保守上界是每个全分辨率像素 24 次纹理读取。
shader-work admission 现按这个真实上界计费，并把总上限设为 350 Mi work units：
经过数值合同验证的 1080p 默认请求仍可进入，已知 1440p/4K 最坏路径继续被拒绝。
这是有界准入修正，不等同于实机性能门通过。
