...
116. **Phase 7.71 路径阻断器：legacy 拦截链路适配新数据层（2026-05-15 03:45）**:
   - **问题**：
     用户实机反馈：`YT??` 系列路径阻断器（YTab/YTac/YTpb/YTpc/YTfb/YTfc/YTlb/YTlc）
     正常游戏不可见，但仍然能投出阴影。原因是新长期路线把对象身份收束到
     `War3RenderState::TlsShadowSemanticState` 桥，rawcode 经常只在 TLS 上有，
     而不在 `RenderObjectRegistry::findByHandle` 里。`War3TryCaptureShadowCaster`
     的旧 LOS 过滤只看 `pathBlockObj->rawcode`，依赖 `currentObj` 或
     RenderObjectRegistry 解析。两条路径阻断器走漏点：
     1. `currentObj == nullptr`（terrain doodad 路径强制清零 currentObj），
        而 RenderObjectRegistry 又没注册到该 jHandle → 整个 LOS 检查被跳过；
     2. `currentObj != nullptr` 但 `rawcode == 0`，registry lookup 也 miss；
        语义桥的 rawcode 留在 `semantic.rawcode` 但旧检查不读它。
   - **修复**（`src/d3d9/d3d9_device.cpp::War3TryCaptureShadowCaster`）：
     - **第一道：函数最早期**。`shadowSemantic` TLS 加载之后立刻看
       `IsLosBlockerFourCc(shadowSemantic.rawcode)` 与
       `IsLosBlockerFourCc(shadowSemantic.object->rawcode)`，命中即 return。
       这一道连 v4 capture / 早期 bypass / legacy 主体三个分支都不会走，
       彻底防止 path blocker 流入 semanticSceneBypassCandidate 或 v4 cache。
     - **第二道：legacy 主体内**。把原先只看 `pathBlockObj->rawcode` 的检查改成
       `War3ShadowIsLosBlocker(semantic, currentObj)` + 原有 batchHandle ->
       RegistryLookup 兜底。和早期 bypass 分支
       (`War3ShadowIsLosBlocker(semantic, earlyCurrentObj)`) 行为对齐。
     - 计数器从 `skippedNotCaster++` 升级到 `semanticSceneRejectedPathBlockerCount++
        + skippedNotCaster++`，与新长期路线的拒绝桶语义一致，full trace 可
       直接读 `semanticSceneRejectedPathBlockerCount` 看本帧拦了多少。
   - **不动的部分**：
     - 不改 producer / draw-time VB cache / Phase 7.70 dedup；
     - 不改 manifest TTL / stale restore / receiver hold；
     - 不动 `IsLosBlockerFourCc` 黑名单条目；
     - `kPathBlockerHideEnabled` 默认值 `true` 不变。
   - **验证**：
     - `ninja -C build32` 通过；
     - 部署 `E:\Work\War3\d3d9.dll = 26767269 bytes @ 2026-05-15 03:42:17`；
     - AutoTest（光影测试.w3x，15s full trace）：
       - 帧数 960
       - `submitted=0` = `0 / 960`
       - `shadowMapExec` = 1.0/帧、`historyValid` = 1.0/帧
       - `producer fallback to current-draw` = 0
       - `path blocker reject` = 3.17/帧（基线 3.16，光影测试地图本身路径阻断器很少，
         主要看实机大量路径阻断器场景的视觉验收）。
     - 无崩溃、无新增警告。
   - **建议用户视觉复核场景**：
     - 树林/山隘/桥头等 War3 地图常见 LOS 阻断器密集区；
     - 第一道拦截的命中量可观察 `semanticSceneRejectedPathBlockerCount` 显著上升；
     - 第二道（legacy）只在 `earlyNeedsSemanticContext == false` 路径上触发；
       通常发生在 terrainCaster=true（`cat==Terrain && stage in {1,10}`）时。
